    ...
    fun start() {
        scope.launch {
            ...
            val audioFormat = AudioFormat(
                ...
                videoGrabber.sampleRate.toFloat() * 2,
                16,
                videoGrabber.audioChannels,
                true,
                true
            )
            ...
            var frame: Frame? = videoGrabber.grab() // The simple grab() function may return audio frame as well
            val startTime = System.nanoTime() / 1000 - frame!!.timestamp

            while (frame != null) {
                while (!isResumed) delay(50.milliseconds)
                if (frame.type == Frame.Type.VIDEO) {
                    val bitmap = imageConverter.convert(frame).toComposeImageBitmap()
                    _videoFrames.value = bitmap
                    val elapsedTime = (System.nanoTime() / 1000) - startTime
                    delay((frame.timestamp - elapsedTime).microseconds)
